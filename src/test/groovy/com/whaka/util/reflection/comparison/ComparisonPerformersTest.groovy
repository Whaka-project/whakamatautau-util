...
import static com.whaka.util.reflection.comparison.ComparisonPerformers.*
...
class ComparisonPerformersTest extends Specification {
	...
	def "from-predicate"() {
		...
		when:
			ComparisonResult result1 = performer.compare(42, "qwe")
		then:
		...
		when:
			ComparisonResult result2 = performer.compare(false, true)
		then:
	...
	def "DEEP_EQUALS"() {
		when:
			ComparisonResult result = DEEP_EQUALS.compare(actual, expected)
		then:
