...
class ReflectiveComparisonPerformerTest extends Specification {
	...
	def "INSTANCE - simple objects"() {
		when:
			def result1 = INSTANCE.compare(actual, expected)
			def result2 = INSTANCE.compare(expected, actual)
		then:
	...
	def "INSTANCE - different classes"() {
		...
		when:
			def result = INSTANCE.compare(actual, expected)
		then:
	...
	def "INSTANCE - complex objects"() {
		when:
			ComparisonResult result = INSTANCE.compare(MARTIN, MARTINA)
		then:
		...
		when:
			...
			ComparisonResult resultPosition = INSTANCE.compare(positionRacer, positionSpy)
		then:
	...
	def "INSTANCE - arrays of complex objects"() {
		...
		when:
			ComparisonResult result = INSTANCE.compare(arr1, arr2)
		then:
