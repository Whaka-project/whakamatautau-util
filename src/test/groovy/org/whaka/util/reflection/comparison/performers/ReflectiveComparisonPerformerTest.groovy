...
class ReflectiveComparisonPerformerTest extends Specification {
	...
	def "INSTANCE - simple objects"() {
		when:
			def result1 = INSTANCE.appl(actual, expected)
			def result2 = INSTANCE.appl(expected, actual)
		then:
	...
	def "INSTANCE - different classes"() {
		...
		when:
			def result = INSTANCE.appl(actual, expected)
		then:
	...
	def "INSTANCE - complex objects"() {
		when:
			ComparisonResult result = INSTANCE.appl(MARTIN, MARTINA)
		then:
		...
		when:
			...
			ComparisonResult resultPosition = INSTANCE.appl(positionRacer, positionSpy)
		then:
	...
	def "INSTANCE - arrays of complex objects"() {
		...
		when:
			ComparisonResult result = INSTANCE.appl(arr1, arr2)
		then:
