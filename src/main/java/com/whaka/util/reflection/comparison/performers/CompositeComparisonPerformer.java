 ...
 *
 * <p>When {@link #compare(Object, Object)} is called - the same method with the same arguments is called
...
public class CompositeComparisonPerformer<T> extends AbstractComparisonPerformer<T> {
	 ...
	 * on {@link #compare(Object, Object)} call!
	 */
	...
	public ComparisonResult compare(T actual, T expected) {
		...
		for (Map.Entry<ClassPropertyKey, ComparisonPerformer<T>> e : getPerformers().entrySet()) {
			ComparisonResult result = e.getValue().compare(actual, expected);
